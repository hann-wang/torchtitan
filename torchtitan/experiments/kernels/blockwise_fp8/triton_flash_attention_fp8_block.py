...
def attn_fwd(
    ...
    if USE_FP8:
        ...
        q_descale_offset = q_descale_ptr + stride_qdescale_z * off_z + stride_qdescale_h * off_h_q + start_m
...
def _bwd_kernel_dkdv(
    ...
    if USE_FP8:
        k_descale_offset = (k_descale_ptr + off_z * stride_kscalez +
                            off_h_k * stride_kscaleh +
                            start_n * stride_kscalem)
        blk_k_descale = tl.load(k_descale_offset)

        v_descale_offset = (v_descale_ptr + off_z * stride_vscalez +
                            off_h_k * stride_vscaleh +
                            start_n * stride_vscalem)
...
def attention_block_backward_triton_impl(
    ...
    if use_fp8:
        ...
        stride_vscalez, stride_vscaleh, stride_vscalem = k_descale.stride()
