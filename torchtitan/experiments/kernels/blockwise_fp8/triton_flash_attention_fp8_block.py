...
def get_tl_f8_bwd_dtype():
    ...
    return tl.float8e4b8 if is_hip() else tl.float8e4nv
...
@triton_op("torchtitan::attention_block_forward_triton_impl", mutates_args=())
...
def _bwd_kernel_dkdv(
    ...
    if USE_FP8:
        q_descale_offset = (q_descale_ptr + off_z * stride_qscalez +
                            off_h_q * stride_qscaleh +
                            q_start * stride_qscalem)
        do_descale_offset = (do_descale_ptr + off_z * stride_doscalez +
    ...
    if USE_FP8:
        ...
        v_descale_offset = v_descale_ptr + off_z * stride_vscalez + off_h_k * stride_vscaleh + start_n * stride_vscalem
...
def _attn_bwd_dkdv(
    ...
    for start_m in range(lo, num_block_m * BLOCK_M, BLOCK_M):
        ...
        # compute dv
        _dv = tl.dot(tl.trans(p.to(k.dtype)),
                     do,
...
def _bwd_kernel_dq(
    ...
    if USE_FP8:
        ...
        k_descale_offset = (
            k_descale_ptr + off_z * stride_kscalez + off_h_k * stride_kscaleh +
            tl.arange(0, padded_kscale_block_num))  #  + q_start * stride_qm
        k_descale = tl.load(k_descale_offset, mask=kscale_mask, other=1.0)
    ...
    if USE_FP8:
        q_descale_offset = q_descale_ptr + off_z * stride_qscalez + off_h_q * stride_qscaleh + start_m * stride_qscalem
...
@triton_op("torchtitan::attention_block_backward_triton_impl", mutates_args=())
def attention_block_backward_triton_impl(
    ...
    grid_bwd = (
        batch_headsize_q,
        triton.cdiv(max_seqlen_q, FIXED_BLOCK_M),
    )
    ...
    grid_bwd_dkdv = (
        batch_headsize_k,
        triton.cdiv(max_seqlen_k, FIXED_BLOCK_N),
    )
...
class _triton_attention_block(torch.autograd.Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        dq, dk, dv = torch.ops.torchtitan.attention_block_backward_triton_impl(
            do,
            q,
            k,
            v,
