...
class GQAttention(BaseAttention):
    ...
    def __init__(self, config: Config):
        ...
        self.qk_norm_type: int = 0  # 0: no norm, 1: Qwen3-style norm, 2: Instella-3B-style norm
        if config.qk_norm is not None:
            self.q_norm = config.qk_norm.build()
            self.k_norm = config.qk_norm.build()
            if config.qk_norm.normalized_shape == self.head_dim:
                self.qk_norm_type = 1
            elif config.qk_norm.normalized_shape == self.n_heads * self.head_dim:
                assert self.n_heads == self.n_kv_heads, "n_heads must be equal to n_kv_heads for Instella-3B-style norm"
                self.qk_norm_type = 2
            else:
                raise ValueError(
                    f"Unknown QK normalization type: {config.qk_norm.normalized_shape}"
                )
    ...
    def forward(
        ...
        if self.qk_norm_type == 2:
            ...
            xq = xq.view(bs, seqlen, -1)
            xk = xk.view(bs, seqlen, -1)
            xv = xv.view(bs, seqlen, -1)
